...
# MAGIC To run this demo, just select the cluster `dbdemos-llm-rag-chatbot-andreas_jack` from the dropdown menu ([open cluster configuration](https://e2-demo-field-eng.cloud.databricks.com/#setting/clusters/1218-171209-zxakb8og/configuration)). <br />
...
# MAGIC %pip install langchain-community==0.0.8 langchain_core==0.1.5 langchain==0.0.354 mlflow==2.9.1 databricks-vectorsearch==0.22 databricks-sdk==0.12.0 mlflow[databricks]
...
# Enable when using foundational api models
#from langchain.chat_models import ChatDatabricks
...
chain = RetrievalQA.from_chain_type(
    llm=chat_model,# pass Llama-2 endpoint
    chain_type="stuff",
    retriever=get_retriever(), # pass our vector index retriever 
